/*
 * Box Platform API
 *
 * [Box Platform](https://box.dev) provides functionality to provide access to content stored within [Box](https://box.com). It provides endpoints for basic manipulation of files and folders, management of users within an enterprise, as well as more complex topics such as legal holds and retention policies.
 *
 * The version of the OpenAPI document: 2.0.0
 * Contact: devrel@box.com
 * Generated by: https://openapi-generator.tech
 */

/// MetadataQueryIndices : A collection of metadata query indices.



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct MetadataQueryIndices {
    /// A collection of metadata query indices.
    #[serde(rename = "entries", skip_serializing_if = "Option::is_none")]
    pub entries: Option<Vec<crate::models::MetadataQueryIndex>>,
    /// The limit that was used for this request.
    #[serde(rename = "limit", skip_serializing_if = "Option::is_none")]
    pub limit: Option<i64>,
    /// The marker for the start of the next page of results.
    #[serde(rename = "next_marker", skip_serializing_if = "Option::is_none")]
    pub next_marker: Option<String>,
}

impl MetadataQueryIndices {
    /// A collection of metadata query indices.
    pub fn new() -> MetadataQueryIndices {
        MetadataQueryIndices {
            entries: None,
            limit: None,
            next_marker: None,
        }
    }
}


